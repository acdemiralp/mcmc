...
public:
  ...
  explicit independent_metropolis_hastings_sampler(
    ...
    const std::function<float(float)>&             log_proposal_density_function,
  ...
  state_type apply (const state_type& state)
  {
    ...
    const auto proposal_given_current = log_proposal_density_function_(density         );
    const auto current_given_proposal = log_proposal_density_function_(current_density_);
...
protected:
  ...
  std::function<float(float)>                                    log_proposal_density_function_;
